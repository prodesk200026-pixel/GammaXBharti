...
const { ema: emaFn, rsi: rsiFn } = require('./indicators');
...
function stageFromState(state) {
  switch (state) {
    case 'IDLE': case 'ARMED_CROSS': return 0;
    case 'IMPULSE': return 1;
    case 'PULLBACK': case 'DOT_ARMED': return 2;
    case 'DOT_MARKED': return 3;
    default: return 0;
  }
}
function trendFromWatcher(watcher) {
  return watcher.lastCrossDirection === 'UP' ? 'RISING' : watcher.lastCrossDirection === 'DOWN' ? 'FALLING' : 'FLAT';
}

/**
 * Builds the exact shape the "RSI-Gann Levels" frontend (app.js -> renderRsiGann)
 * expects from GET /api/rsigann/:symbol and the /ws/market push messages.
 */
function buildRsiGannPayload(idx) {
  const rt = runtime[idx];
  if (!rt) return { ready: false, reason: 'Unknown symbol' };
  const stratCfg = rt.call.cfg;
  const candles = rt.candles;
  const minBars = Math.max(stratCfg.rsiPeriod, stratCfg.emaSlowPeriod) + stratCfg.rsiEmaPeriod + 5;
  if (!candles || candles.length < minBars) {
    return { ready: false, reason: 'Waiting for enough live bars…' };
  }

  const closes = candles.map(c => c.c);
  const rsiArr = rsiFn(closes, stratCfg.rsiPeriod);
  const rsiEmaArr = emaFn(rsiArr.map(v => (v === null ? closes[0] : v)), stratCfg.rsiEmaPeriod);
  const fastArr = emaFn(closes, stratCfg.emaFastPeriod);
  const slowArr = emaFn(closes, stratCfg.emaSlowPeriod);
  const i = candles.length - 1;

  const bullStage = stageFromState(rt.call.state);
  const bearStage = stageFromState(rt.put.state);

  const recentAlerts = rt.status.alerts.slice(0, 5);
  const bullConfirmDot = recentAlerts.some(a => a.kind === 'STRATEGY' && a.type === 'GREEN_DOT' && a.direction === 'CALL');
  const bearConfirmDot = recentAlerts.some(a => a.kind === 'STRATEGY' && a.type === 'GREEN_DOT' && a.direction === 'PUT');
  const recentEntry = recentAlerts.find(a => a.kind === 'ENTRY_CARD');
  const showEntry = recentEntry && (Date.now() - recentEntry.at < 60000); // surface a BUY verdict for 60s after it fires

  let verdict = 'WAIT', strike = null;
  if (showEntry) {
    verdict = recentEntry.direction === 'CALL' ? 'BUY_CALL' : 'BUY_PUT';
    strike = (recentEntry.strikePick && recentEntry.strikePick.strike) || rt.status.atmStrike;
  }

  const straddleTrend = trendFromWatcher(rt.straddleW);
  const ivTrend = trendFromWatcher(rt.ivW);

  return {
    ready: true,
    verdict,
    strike,
    price: rt.status.underlyingPrice,
    rsi: rsiArr[i],
    rsiSignal: rsiEmaArr[i],
    emaFast: fastArr[i],
    emaSlow: slowArr[i],
    bullStage, bearStage, bullConfirmDot, bearConfirmDot,
    straddleTrend, ivTrend,
    // Soft heuristic, matches the frontend's own "not a hard gate" framing —
    // volatility expanding in the direction of the move is the classic
    // pre-breakout tell; it isn't a precise confirmation signal.
    volConfirmation: straddleTrend === 'RISING' && ivTrend === 'RISING',
  };
}

function broadcastMarketUpdate(idx, payload) {
  if (!wss.clients.size) return;
  const msg = JSON.stringify({ type: 'market_update', symbol: idx, rsiGannSignal: payload });
...
async function pollAll() {
  ...
  for (const idx of cfg.trackIndices) {
    try {
      await pollIndex(idx);
      broadcastMarketUpdate(idx, buildRsiGannPayload(idx));
    } catch (e) { console.error(`[${idx}] poll error`, e.message); }
  }
}
...
// The "RSI-Gann Levels" PWA (app.js) calls these exact paths — no
// x-api-secret header is ever sent by that frontend, so these stay
// open even if API_SECRET is set for the other /api/* routes above.
app.get('/api/rsigann/:symbol', (req, res) => {
  const idx = req.params.symbol.toUpperCase();
  if (!runtime[idx]) return res.status(404).json({ error: 'unknown symbol', known: cfg.trackIndices });
  res.json({ rsiGann: buildRsiGannPayload(idx) });
});
app.get('/api/config', (req, res) => res.json({ vapidPublicKey: cfg.vapid.publicKey }));
app.post('/api/push/subscribe', (req, res) => { store.addSubscription(req.body); res.json({ ok: true }); });
app.post('/api/push/test', async (req, res) => {
  await notify('🔔 Test Push', 'This is a test notification from your backend.');
  res.json({ ok: true });
});
...
httpServer.listen(cfg.server.port, () => {
  console.log(`Backend listening on :${cfg.server.port} | tracking ${cfg.trackIndices.join(', ')} | broker=ANGEL_ONLY | ws=/ws/market`);
  pollAll();
